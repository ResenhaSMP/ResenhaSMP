...
The free Blueprints use an experimental reduced Java profile with a 256 MB maximum heap, a 128 MB initial heap, a 96 MB Metaspace cap, a 48 MB direct-memory cap, a 12 MB code-cache cap, SerialGC, a 256 KB thread stack, one active processor, and `MALLOC_ARENA_MAX=1` so the process has room under Render's 512 MB instance limit. They also pre-patch Paper 1.12.2 build 1620 during the Docker image build and copy the validated Paperclip cache into `/data` before startup, avoiding the high-memory patch operation during the free runtime boot. This profile is intended only to test whether the server can run on the free instance; it is not guaranteed to work and is not suitable for a persistent public survival server. Render may suspend a free web service after 15 minutes without inbound traffic, and local filesystem changes can be lost after suspension, restart, or redeploy. That means the world, accounts, plugins, and generated configuration may disappear.
